...
__global__ void cuda_mdot_product(int N, double *x, double *y, double *res)
{
  ...
  if (threadIdx.x == 0)
  {
    atomicAdd(&res[0], shared_mem1[0]);
    atomicAdd(&res[1], shared_mem2[0]);
    atomicAdd(&res[2], shared_mem3[0]);
    atomicAdd(&res[3], shared_mem4[0]);
    atomicAdd(&res[4], shared_mem5[0]);
    atomicAdd(&res[5], shared_mem6[0]);
    atomicAdd(&res[6], shared_mem7[0]);
    atomicAdd(&res[7], shared_mem8[0]);
  }
}
